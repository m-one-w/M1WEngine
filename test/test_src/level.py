...
from game_data import level_data, character_keys, item_keys
...
class Level(object):
    ...
    _extra_sprites: pygame.sprite.Group
    ...
    Methods
    -------
    create_map(self, level_dict: dict) -> None
        Parses gameData.py's level_data using level_key to get this level's sprites
    create_sprite_groups(self) -> None
        Create all sprites groups used in the level
    create_entities_from_layout(self, layout: List[int]) -> None
    ...
    create_items_from_layout(self) -> None
        Create all items
    add_obstacles(self) -> None
        Add level specific obstacles to _obstacle_sprites
    add_sprites_to_camera(self) -> None
        Add all visible sprites to the CameraManager
    run(self) -> None
        Draw and update all sprite groups
    """

    def __init__(self, universal_assets: list[pygame.Surface], level_str: str) -> None:
    ...
    def create_map(
        ...
        ----------
        level_key: str
            The key to select the correct level
        universal_assets: list[pygame.Surface]
